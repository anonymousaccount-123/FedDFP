...
def get_tissue_mask(args):
    for slide_pth in tqdm(os.listdir(args.wsi_path)):
        if slide_pth.endswith('.tif'):
            slide_pth_full = os.path.join(args.wsi_path, slide_pth)
            mask_save_pth = os.path.join(args.npy_path, slide_pth[:-4] + '.npy')
            slide = openslide.OpenSlide(slide_pth_full)
            ...
            np.save(mask_save_pth, tissue_mask)

def get_probs_map(args):
