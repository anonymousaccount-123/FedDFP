...
def distribution_matching_woMIL_new(image_real, image_syn, optimizer_img, channel, num_classes, im_size, ipc, nps, args=None, loss_fn=None, indices_class=None):
    net = resnet50_baseline(pretrained=True).to(device).eval()
    ...
    for c in range(num_classes):
        ...
        if output_real.size(0) == 0:
            continue
        c_idx = indices_class[c]
        n_slide_cls, n_patch_slide, channel, h, w = image_syn[c_idx].size()
        img_syn = image_syn[c_idx].reshape((n_slide_cls*n_patch_slide, channel, im_size, im_size))
        syn_loader = DataLoader(TensorDataset(img_syn), batch_size=n_patch_slide, shuffle=True)
        output_syn = []
        step = 0
        for img_syn in syn_loader:
            img_syn = img_syn[0].to(device)
